...
const saveRating = async () => {
  ...
  try {
    ...
    // oder auf die richtige Eigenschaft zugreifen
    const titleData = title.value || {};
    
    // Versuche alternative Wege, die ID zu finden (verschiedene Eigenschaftsnamen prüfen)
    ...
    if (tmdbIdValue === null) {
      // Versuche tiefer zu schauen, falls es ein verschachteltes Objekt ist
    ...
    // Erstelle das zu sendende Objekt mit der gefundenen ID
    const ratedTitle = {
      tmdbId: tmdbIdNumber,
      name: titleData.name || titleData.title || "",
      type: route.params.type === 'movie' ? 'movie' : 'series',
    ...
    const response = await fetch('/api/titles/rate', {
      method: 'POST',
      headers: {
